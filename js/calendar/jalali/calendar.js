...
Calendar.cellClick = function(el, ev) {
	...
	if (typeof el.navtype == "undefined") {
		...
		date = new CalendarDateObject(cal.date);
		if (el.navtype == 0)
			date.setUTCDateOnly(new CalendarDateObject()); // TODAY
...
Calendar.prototype.create = function (_par) {
	...
	if (!this.date) this.date = this.dateStr ? new CalendarDateObject(this.dateStr) : new CalendarDateObject();
...
Date.parseDate = function(str, format, dateType) {
	str = str.toEnglish();
	var today = new CalendarDateObject();
	var result = new CalendarDateObject();
	var y = null;
	var m = null;
	var d = null;
	var hr = 0;
		...
		if (a[i].charAt(0) == '%') {
			switch (a[i]) {
				...
				case "%s":
					var time;
					if (time = str.match(/^-?\d+/)) {
						return new CalendarDateObject(parseInt(time[0], 10) * 1000);
					}
					break;
...
Date.prototype.getUTCDayOfYear = function() {
	var now = new CalendarDateObject(Date.UTC(this.getUTCFullYear(), this.getUTCMonth(), this.getUTCDate(), 0, 0, 0));
	var then = new CalendarDateObject(Date.UTC(this.getUTCFullYear(), 0, 0, 0, 0, 0));
	var time = now - then;
...
Date.prototype.getJalaliUTCDayOfYear = function() {
	...
	var then = new CalendarDateObject(Date.UTC(j[0], j[1]-1, j[2], 0, 0, 0));
	var time = now - then;
	return Math.floor(time / Date.DAY);
};
...
Date.prototype.getUTCWeekNumber = function() {
	var d = new CalendarDateObject(Date.UTC(this.getUTCFullYear(), this.getUTCMonth(), this.getUTCDate(), 0, 0, 0));
	var DoW = d.getUTCDay();
...
Date.prototype.getJalaliUTCWeekNumber = function() {
	...
	var d = new CalendarDateObject(Date.UTC(j[0], j[1]-1, j[2], 0, 0, 0));
	
	//Number of days after the first Saturday of the year
	var days = this.getJalaliUTCDayOfYear() - ((7 - d.getJalaliUTCDay()) % 7) - 1;
	
	if (days < 0) return new CalendarDateObject(this - this.getJalaliUTCDay()*Date.DAY).getJalaliUTCWeekNumber();
...
Date.prototype._calSetFullYear = function(y) {
	var date = new CalendarDateObject(this);
